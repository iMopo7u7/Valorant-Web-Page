...
app.post("/players", requireAdmin, async (req, res) => {
  try {
    ...
    const newPlayer = {
      ...
      matchesPlayed: 0, wins: 0, badges, social, score: 0
    };
...
app.get("/players", requireAdmin, async (req, res) => {
  try { const players = await playersCollection.find().toArray(); res.json(players); }
  catch (err) { console.error(err); res.status(500).json({ error: "Error al obtener jugadores" }); }
});
...
app.put("/players", requireAdmin, async (req, res) => {
  ...
  try {
    await playersCollection.updateOne(
      ...
      { $set: { name: newName, tag: newTag, social } }
    );
    res.json({ message: "Jugador actualizado" });
  } catch (err) { console.error(err); res.status(500).json({ error: "Error actualizando jugador" }); }
});
...
app.delete("/players", requireAdmin, async (req, res) => {
  ...
  } catch (err) { console.error(err); res.status(500).json({ error: "Error eliminando jugador" }); }
});
    ...
    for (const p of match) {
      ...
      await playersCollection.updateOne(
        ...
        { $inc: {
            ...
            totalACS: p.acs, totalFirstBloods: p.firstBloods,
...
app.get("/matches", requireAdmin, async (req, res) => {
  try {
    ...
    res.json(matches);
  } catch (err) { console.error(err); res.status(500).json({ error: "Error obteniendo partidas" }); }
});
...
app.put("/matches/:id", requireAdmin, async (req, res) => {
  ...
  } catch (err) { console.error(err); res.status(500).json({ error: "Error actualizando partida" }); }
});
...
app.delete("/matches/:id", requireAdmin, async (req, res) => {
  ...
  } catch (err) { console.error(err); res.status(500).json({ error: "Error eliminando partida" }); }
});
...
app.get("/leaderboard", async (req, res) => {
  try { const players = await playersCollection.find().toArray(); players.sort((a, b) => (b.score || 0) - (a.score || 0)); res.json(players); }
  catch (err) { console.error(err); res.status(500).json({ error: "Error generando leaderboard" }); }
});
...
app.get("/last-match", async (req, res) => {
  ...
  } catch (err) { console.error(err); res.status(500).json({ error: "Error al obtener última partida" }); }
});
